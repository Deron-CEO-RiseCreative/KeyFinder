...
- (void)viewDidLoad {
    ...
    _btnViewLock.layer.cornerRadius = 6;
...
-(IBAction)didClickButtonViewLock:(id)sender{
    if (_bFromLock == YES)
        ...
        [self performSegueWithIdentifier:@"segueLockView"
                              sender:Nil];
}

-(IBAction)didClickButtonKeyMap:(id)sender{
    [self performSegueWithIdentifier:@"segueKeyMapView"
                              sender:Nil];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([[segue identifier] isEqualToString:@"segueLockView"]) {
        LockViewController* vcKeyView = (LockViewController*)[segue destinationViewController];
